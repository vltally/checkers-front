...
export default function Chessboard() {
    ...
    function grabPiece(e: React.MouseEvent) {
        ...
        if (element.classList.contains('chess-piece') && chessboard) {
            ...
            if (mandatoryCaptures.length > 0) {
                const isMandatoryPiece = mandatoryCaptures.some((pos) =>
                    samePosition(pos, { x: grabX, y: grabY })
                );
                if (!isMandatoryPiece) {
                    return;
                }
            ...
            if (currentPiece) {
                ...
                if (moveResult.success) {
                    let updatedPieces = pieces.map((piece) => {
                        ...
                        if (
                            samePosition(piece.position, currentPiece.position)
                        ) {
    ...
    for (let j = VERTICAL_AXIS.length - 1; j >= 0; j--) {
        for (let i = 0; i < HORIZONTAL_AXIS.length; i++) {
            const number = j + i + 2;
            let image = undefined;
            const isHighlighted = mandatoryCaptures.some((pos) =>
                samePosition(pos, { x: i, y: j })
            );
            const piece = pieces.find((p) =>
